      ...
      <div class="warp-z">
        ...
        >
          ...
          <div class="mesWarp" v-for="(item,index) in listdata">
            ...
            <router-link :to="{name:'ItemsDetail',query:{signerId:item.id}}">
              <div class="message line">
                <p class="site">人员类型：{{item.populationTypeName}}</p>
                <p class="warn">
                  预警等级：
                  <img
                    class="warn_ico"
            ...
            <div class="evaluate">
              <router-link :to="{name:'ServiceView',query:{signerId:item.id}}">
                <span class="eval btn_common">查看</span>
              </router-link>
              <router-link
                :to="{name:'Perform',query:{signerId:item.id,chooseOrganizationId:chooseOrganizationId}}"
              >
                <span class="sign_btn btn_common" v-show="getMore&&authorityIs">执行</span>
              </router-link>
            </div>
          </div>
          <!-- </pull-refresh> -->
...
export default {
  ...
  methods: {
    ...
    getCurrentLists() {
      var api =
        "/plugin/psychosocialService/serviceItems/findServiceitemsForMobile.action";
      this.$http
        .post(api, {
          // 当前所选择的组织机构id
          "serviceItemsVo.organization.id": this.chooseOrganizationId,
          //当前列表使用sameGrade、下辖列表使用directJurisdiction
          "serviceItemsVo.orgScope": this.orgScope,
          // 当前登录对象所在层级 全国60、省50、市40、区县30、街道20、村10、网格0
          "serviceItemsVo.currentUserOrgLevel": this.internalId,
          // rows: 10,
          page: 1
        })
        .then(
        ...
        if (Math.ceil(this.sumtotal / 10) < this.page) {
          ...
          this.$http
            .post(api, {
              ...
              "serviceItemsVo.currentUserOrgLevel": this.internalId,
             // 完成时间
              'serviceItemsVo.beginDate':this.date,
              // 截至时间
              'serviceItemsVo.endDate':this.dateEnd,
              // 本次只做精神病人员
              "serviceItemsVo.populaitionType":this.mentalPatient,
              // 预警等级
              'serviceItemsVo.warningLevel.id':this.warningLevelId,
              // 方案名称
              'serviceItemsVo.schemeName':this.planName,
              page: this.page
            })
            .then(
              response => {
    ...
    authority() {
      this.$http
        .post(
          ...
          {
            enamesStr: "serviceItemsImplement"
          }
        )
        .then(res => {
          this.authorityIs = res.data.serviceItemsImplement;
